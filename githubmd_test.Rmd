...
```{r cars, echo=FALSE, fig.height=3, fig.width=3}
...
# )
# y <- list(
#   title = "Catches in 1000 t"
# )
#   
#  p1 <- plot_ly(data, x = ~Year, y = ~catches, type = 'bar', showlegend = T)%>%
#           layout(title= "Catches", xaxis = x, yaxis = y)
#  
#  x <- list(
#   title = ""
# )
# y <- list(
#   title = "Recruitment in millions"
# )
#   
#  p2 <- plot_ly(data, x = ~Year, y = ~recruitment, type = 'bar', showlegend = T)%>%
#           layout(title= "Recruitment (age 0)", xaxis = x, yaxis = y)
#  
#  subplot(p1,p2)
 
 a<-dygraph(catches, main = "Catches")%>%
   dyRangeSelector()%>%
   dyOptions(colors = RColorBrewer::brewer.pal(3, "Set2"), drawGrid = FALSE,maxNumberWidth = 4)%>%
   dyAxis("y", label = "Catches in 1000 t")%>%
   dyBarChart()
 
 b<-dygraph(recruitment, main = "Recruitment(age 0)") %>%
   dyRangeSelector()%>%
...
```{r cars2, echo=FALSE, fig.height=3, fig.width=3}
 library(htmlwidgets)
 library(dplyr)
 library(ggplot2)
 library(gridExtra)
