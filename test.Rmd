...
library(plotly)
 ...
 p1 <- plot_ly(data, x = ~F) %>%
          add_trace(y = ~ SSB, name= 'SSB 2019', mode = 'lines+markers')%>%
          add_trace(y = ~ Catch, name = 'Catch 2018', mode = 'lines+markers')%>%
...
vis<- gvisMotionChart(hke, idvar = "StockCode", timevar = "Year", xvar = "SSB.MSYBtrigger",
  yvar = "F.Fmsy", colorvar = "EcoRegion", sizevar = "", date.format = "%Y/%m/%d",
  options = list())
vis
```

```{rgooglevis results='asis'}
print(vis,'chart')
```
