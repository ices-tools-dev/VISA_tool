...
\*** Not provided: TAC 2019 not predicted because of the imbroglio between the advised catch (for EU and Faroese waters) and the EU
TAC.
\^ Advice value 2020 relative to advice value 2019.
\^^ F driving the stock down to Bpa, Blim, and MSY Btrigger in 2021 when applied in 2020.
...
<br>
```{r catchoptionsgraph1, echo = FALSE,fig.height = 4.5, fig.width = 10}
library(ggplot2)
library(plotly)
 catchoptions <- read.csv("Data/bli/bliscenariosplot1.csv")
 labels <- catchoptions$Basis
 labels <- as.character(labels)
 mypalette<-ggthemes::tableau_color_pal('tableau20')
 
 data <- catchoptions[order(catchoptions$F2019),] 
 data$my_text=paste(data$Type , data$Basis, sep=": ")
 
 p1 <- plot_ly(data, x = ~F2019) %>%
    ...
    annotations=
        ...
        add_trace(y = ~ Catch2019, name = 'Catch 2019', mode = 'lines+markers',text = data$my_text, hoverinfo = "text + x+ y")
...
## Scenarios for 2020:
<div class="col2">
A. With F2019 = FMSY                                     B. With F2019 = Fsq = F2017
```{r catchoptionsgraph2, fig.height=4.5, fig.width=4.5}
library(ggplot2)
library(plotly)
 catchoptions <- read.csv("Data/bli/bliscenariosplot2.csv")
 labels <- catchoptions$Basis
 labels <- as.character(labels)
 mypalette<-ggthemes::tableau_color_pal('tableau20')
 
 data <- catchoptions[order(catchoptions$F2020),] 
 data$my_text=paste(data$Type , data$Basis, sep=": ")
 
 p1 <- plot_ly(data, x = ~F2020) %>%
    ...
    annotations=
        list(
         ...
         add_trace(y = ~ SSB_2021, name= 'SSB 2021', mode = 'lines+markers',text = data$my_text, hoverinfo = "text + x+ y") %>%
        add_trace(y = ~ Catch2020, name = 'Catch 2020', mode = 'lines+markers',text = data$my_text, hoverinfo = "text + x+ y")
                  
  p1
```
 
 B. With F2019 = Fsq = F2017
```{r catchoptionsgraph2b, fig.height=4.5, fig.width=4.5}
library(ggplot2)
library(plotly)
 catchoptions <- read.csv("Data/bli/bliscenariosplot3.csv")
 labels <- catchoptions$Basis
 labels <- as.character(labels)
 mypalette<-ggthemes::tableau_color_pal('tableau20')
